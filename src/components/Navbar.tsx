...
export function Navbar() {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 border-b transition ${scrolled || open ? "border-white/10 bg-[#08070c]/90 backdrop-blur-xl" : "border-transparent bg-transparent"}`}
    >
      <nav
      ...
      >
        <Link
          ...
          className="font-display text-lg font-black tracking-[.08em] text-white"
        >
        ...
        <div className="hidden items-center gap-7 md:flex">
          {location.pathname === "/" ? (
            ...
            <Link to="/" className="text-sm font-semibold text-white/70">
              Volver al inicio
            </Link>
          )}
          <Link
      ...
      {open && (
        <div className="border-t border-white/10 bg-[#08070c] px-5 py-5 md:hidden">
          <div className="mx-auto flex max-w-7xl flex-col gap-1">
            {links.map(([label, href]) => (
              <a
                ...
                href={`/${href}`}
                onClick={() => setOpen(false)}
                className="rounded-xl px-4 py-3 font-semibold text-white/75 hover:bg-white/5"
              >
                {label}
              </a>
            ))}
            <Link
              to="/ruleta"
              onClick={() => setOpen(false)}
              className="rounded-xl px-4 py-3 font-semibold text-white/60"
            >
              Ruleta
            </Link>
            <Link
